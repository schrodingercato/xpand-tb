...
_Note: Jika Juri hanya ingin melihat-lihat tampilan UI secara cepat tanpa butuh server AI (Mode Mock UI), Juri bisa kapan saja mengunjungi https://xpand-tb.vercel.app dan mengosongkan ikon geriginya._

⚠️ *Disclaimer: Status Mode Demo Cepat (Bypass)*

Secara bawaan (default), repositori backend ini disetel untuk berjalan dalam *Mode Demo Cepat (Bypass Dummy)*.
Sistem tetap memproses data dari ujung-ke-ujung secara nyata (menyimpan ke database, antrean worker di background, dll), namun proses inferensi AI aslinya sengaja di-bypass dan diganti dengan sampel data 3D yang sudah diproses sebelumnya.

*Mengapa demikian?*
1. *Ukuran Model Raksasa:* Model AI aslinya berukuran belasan gigabyte. Memaksa tester untuk mendownloadnya di jaringan lomba sangat berisiko.
2. *Blokir Jaringan:* Situs penyimpanan AI utama (HuggingFace) sering diblokir oleh ISP publik di Indonesia.
3. *Keterbatasan Hardware:* Tidak semua PC Juri memiliki GPU kuat untuk merender 3D dalam hitungan detik.

Mode Bypass ini memastikan Juri tetap bisa menguji kestabilan arsitektur secara utuh (UI, Komunikasi Server, dan Penampil 3D) tanpa halangan instalasi raksasa di lapangan.
